#ifndef __btModels_h_
#define __btModels_h_
...
/**
\file btModels.h
\brief Header file for btModels.cpp
...
\brief The btModels class is simply a class for easy construction of a few 3-D
mathematical objects.

Use of this class is straight forward.  Declare a btModels object, call one of the create
...
// Create and load the data into OpenGL
obj.createSphereOBJ(0.25, 7, 7);
obj.load(0, 1, 2, 3);

// Load the data into Bullet
btConvexHullShape* objPts = new btConvexHullShape();
obj.btLoadConvexHull(objPts);
collisionShapes.push_back(objPts);
...
public:
    ...
    void btLoadTriangleMesh(btTriangleMesh* tm);
    void btLoadConvexHull(btConvexHullShape* chs);

    void scaleObj(float s);
    void translateObj(float x, float y, float z);
    void rotateObj(float x, float y, float z, float ang);

    void createMidpointDisplacementOBJ(GLint grid, GLfloat lx, GLfloat ux,
                                       GLfloat ly, GLfloat uy,
                                       GLfloat mid, GLfloat pert, GLint smoothfact, GLfloat H);

    void createMidpointDisplacementUniformOBJ(GLint grid, GLfloat lx, GLfloat ux,
            GLfloat ly, GLfloat uy,
            GLfloat mid, GLfloat bd, GLint smoothfact);

    btScalar getMax(int p);
    btScalar getMin(int p);
    btScalar getMax(int mp, int xlb, int xub, int ylb, int yub);
    btScalar getMin(int mp, int xlb, int xub, int ylb, int yub);
};
